...
async function postData(payload, setProgress, url, model_path){
...
function UploadData({sucsess, failed, update}){
    ...
    const { values } = useFormikContext()
    const {dbUrl, model_path} = useContext(ConfigContext)
    const [progress, setProgress] = useState([0, 0])
    const {data, isLoading, isError, isFetching, isRefetching, status, error} = useQuery('data', () => postData(createPayload(values), setProgress, dbUrl, model_path), {enabled: !sucsess && !failed})
